...
与 \LaTeX{} 的习惯有所差异。
本模板基于 \pkg{mpt2} 配置数学符号，并尽量遵循国标的规定。

本模板中数学符号的用法与ustctug/ustcthesis有些区别：
\begin{itemize}
  \item 数学常数和特殊函数使用正体，
    如圆周率 $\uppi$、$\upGamma$ 函数。
    应在前面加上\cs{up} 命令转为正体，
    如 \verb|\uppi|。
  \item 向量和矩阵粗斜体，应使用 \cs{bm} 命令，
    如 \verb|\bm{u}|、\verb|\bm{A}|。
  \item 有限增量符号 $\upDelta$ （U+2206）应使用 \cs{upDelta} 命令。
  \item 符号 $\nbl$ ，本模板提供了 \cs{nbl} 命令。
  \item 符号 $\partial$ ，请使用 \cs{partial} 命令或 \cs{vardelta}。
\end{itemize}
...
而且还方便根据需要统一修改改格式，比如：
\begin{itemize}
  \item 向量 $\vec{x}$：\verb|\renewcommand\vec{\bm}|
  \item 矩阵 $\mat{A}$：\verb|\newcommand\mat{\bm}|
  \item 张量 $\ts{T}$： \verb|\newcommand\ts{\bm}|
\end{itemize}
...
另外，公式引用可使用\pkg{amsmath}自带的命令，\cs{eqref}，例如\eqref{1}。
